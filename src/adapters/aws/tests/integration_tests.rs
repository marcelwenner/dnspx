...
use crate::config::models::{AppConfig, AwsAccountConfig, AwsGlobalConfig, AwsServiceDiscoveryConfig};
use crate::core::dns_cache::{DnsCache, CacheKey};
...
pub(crate) async fn verify_dns_cache_contains_entries(
    ...
    for dns_name in expected_dns_names {
        ...
        if entry.is_none() {
            return Err(format!("Expected DNS cache entry for {} not found", dns_name));
        }
    }
    Ok(())
}
...
mod tests {
    ...
    fn test_create_test_aws_discovered_endpoint() {
        let endpoint = create_test_aws_discovered_endpoint(
            "test-instance", 
            "10.0.1.100", 
            "us-east-1", 
            "EC2"
        ...
        assert_eq!(endpoint.service_dns_name, "test-instance.us-east-1.amazonaws.com");
        assert_eq!(endpoint.private_ips.len(), 1);
        assert_eq!(endpoint.private_ips[0], IpAddr::from_str("10.0.1.100").unwrap());
    ...
    fn test_create_minimal_aws_config() {
        ...
        assert_eq!(aws_config.accounts[0].scan_regions, Some(vec!["us-east-1".to_string()]));
    ...
    fn test_aws_global_config_construction() {
        ...
        assert_eq!(global_config.credential_cache_ttl, Duration::from_secs(50 * 60));
