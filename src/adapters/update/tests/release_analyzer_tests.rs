...
- Update dependencies for security patches
        ...
        assert!(result.is_ok(), "Should successfully analyze simple release notes");
        
        let metadata = result.unwrap();
        assert_eq!(metadata.version, Version::new(1, 2, 3));
        assert!(!metadata.has_breaking_changes, "Should not detect breaking changes");
...
- Security patch for authentication bypass
        ...
        let result = analyzer.analyze_release_notes(security_notes, "1.2.4").unwrap();
...
- 💥 Breaking: Update minimum supported version
        ...
        let result = analyzer.analyze_release_notes(breaking_notes, "2.0.0").unwrap();
    ...
    fn test_should_auto_update_patch_only() {
        ...
        let metadata = analyzer.analyze_release_notes("- Bug fix", "1.2.4").unwrap();
        
        let should_update = analyzer.should_auto_update(&policy, &current, &patch_update, &metadata);
        assert!(should_update, "Should allow patch updates with PatchOnly policy");
        
        // Test minor updates blocked
        let minor_update = Version::new(1, 3, 0);
        let minor_metadata = analyzer.analyze_release_notes("- New feature", "1.3.0").unwrap();
        
        let should_update_minor = analyzer.should_auto_update(&policy, &current, &minor_update, &minor_metadata);
        assert!(!should_update_minor, "Should block minor updates with PatchOnly policy");
    }
    ...
    fn test_should_auto_update_minor_and_patch() {
        ...
        let minor_update = Version::new(1, 3, 0);
        let metadata = analyzer.analyze_release_notes("- Add new feature", "1.3.0").unwrap();
        
        let should_update = analyzer.should_auto_update(&policy, &current, &minor_update, &metadata);
        assert!(should_update, "Should allow minor updates with MinorAndPatch policy");
        
        // Test major updates blocked
        let major_update = Version::new(2, 0, 0);
        let major_metadata = analyzer.analyze_release_notes("- Major version release", "2.0.0").unwrap();
        
        let should_update_major = analyzer.should_auto_update(&policy, &current, &major_update, &major_metadata);
        assert!(!should_update_major, "Should block major updates with MinorAndPatch policy");
    }
    ...
    fn test_should_auto_update_security_override() {
        ...
        let security_metadata = analyzer.analyze_release_notes("- Security fix for vulnerability", "1.3.0").unwrap();
        
        let should_update = analyzer.should_auto_update(&policy, &current, &security_minor, &security_metadata);
        assert!(should_update, "Should allow security updates even if they exceed policy level");
    }
    ...
    fn test_should_not_auto_update_downgrades() {
        ...
        let metadata = analyzer.analyze_release_notes("- Rollback release", "1.2.2").unwrap();
    ...
    fn test_should_not_auto_update_breaking_changes() {
        ...
        let breaking_metadata = analyzer.analyze_release_notes("💥 Breaking: Remove old API", "2.0.0").unwrap();
        
        let should_update = analyzer.should_auto_update(&policy, &current, &breaking_update, &breaking_metadata);
        assert!(!should_update, "Should block breaking changes even with All policy");
    }
    ...
    fn test_has_breaking_changes_by_version() {
        ...
        assert!(!analyzer.has_breaking_changes(&v1_2_3, &v1_2_4), "Patch updates should not be breaking");
        assert!(!analyzer.has_breaking_changes(&v1_2_3, &v1_3_0), "Minor updates should not be breaking");
        assert!(analyzer.has_breaking_changes(&v1_2_3, &v2_0_0), "Major updates should be breaking");
    }
}