...
mod tests {
    ...
    async fn test_prepare_backup_enabled() {
        ...
        assert!(result.is_ok(), "Prepare should succeed when rollback is enabled");
    ...
    async fn test_prepare_backup_disabled() {
        ...
        let result = transaction.prepare().await;
        assert!(result.is_ok(), "Prepare should succeed when rollback is disabled");
        
        // Should not be available when disabled
        assert!(!transaction.is_rollback_available(), "Rollback should not be available when disabled");
    }
    ...
    async fn test_rollback_when_disabled() {
        ...
        match result.unwrap_err() {
            crate::core::error::UpdateError::RollbackFailed(msg) => {
                assert!(msg.contains("disabled"), "Error should mention rollback is disabled");
            }
    ...
    async fn test_rollback_without_prepare() {
        ...
        match result.unwrap_err() {
            crate::core::error::UpdateError::RollbackFailed(msg) => {
                assert!(msg.contains("backup"), "Error should mention missing backup");
            }
            _ => panic!("Expected RollbackFailed error"),
        }
    }
    ...
    async fn test_is_rollback_available_states() {
        ...
        assert!(!transaction.is_rollback_available(), "Rollback should not be available initially");
    ...
    async fn test_transaction_version_tracking() {
        ...
        assert!(true, "Transaction should handle version parameters correctly");
    }
}