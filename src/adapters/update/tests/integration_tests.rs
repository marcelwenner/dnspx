...
use crate::config::models::{UpdateAutoPolicy, UpdateLevel, UpdateRollbackConfig, UpdateSecurityConfig};
use reqwest::Client;
use semver::Version;
...
async fn create_test_transaction(temp_dir: &tempfile::TempDir) -> UpdateTransaction {
    ...
    let current_binary = create_test_binary(temp_dir.path(), "dnspx-current", &current_binary_content).await;
...
- Update dependencies for security
        ...
        let metadata = analyzer.analyze_release_notes(release_notes, "1.1.0").unwrap();
        ...
        let should_update = analyzer.should_auto_update(&policy, &current_version, &metadata.version, &metadata);
        assert!(should_update, "Should allow minor update with MinorAndPatch policy");
    }
...
- Change configuration format
        ...
        let metadata = analyzer.analyze_release_notes(breaking_notes, "2.0.0").unwrap();
        assert!(metadata.has_breaking_changes, "Should detect breaking changes");
...
- Security patch for authentication bypass
        ...
        let metadata = analyzer.analyze_release_notes(security_notes, "1.0.1").unwrap();
        ...
        let current_version = Version::new(1, 0, 0);
        let should_update = analyzer.should_auto_update(&restrictive_policy, &current_version, &metadata.version, &metadata);
        // Note: Current implementation allows security fixes to override policies within same major version
        assert!(should_update, "Security fixes should override restrictive policies");
    }
    ...
    async fn test_transaction_backup_and_rollback_flow() {
        ...
        assert!(transaction.is_rollback_available(), "Rollback should be available after prepare");
        
        // Create a new binary for commit
        let new_binary_content = create_test_executable_content("1.1.0");
        let new_binary_path = create_test_binary(temp_dir.path(), "dnspx-new", &new_binary_content).await;
    ...
    async fn test_version_comparison_logic() {
        ...
        assert!(!analyzer.has_breaking_changes(&v1_0_0, &v1_0_1), "Patch should not be breaking");
        assert!(!analyzer.has_breaking_changes(&v1_0_0, &v1_1_0), "Minor should not be breaking");
        assert!(analyzer.has_breaking_changes(&v1_0_0, &v2_0_0), "Major should be breaking");
        ...
        let downgrade_metadata = analyzer.analyze_release_notes("Rollback release", "0.9.0").unwrap();
        let should_downgrade = analyzer.should_auto_update(&policy, &v1_0_0, &downgrade_metadata.version, &downgrade_metadata);
...
- Improve input validation for DNS queries
        ...
        let should_update = analyzer.should_auto_update(&policy, &current_version, &metadata.version, &metadata);
        assert!(should_update, "Should allow minor update with security fixes");
        ...
        let test_binary_path = create_test_binary(temp_dir.path(), "test-download", &test_binary_content).await;
        
        let validation_result = security_validator.validate_download(&test_binary_path, None, None).await;
        assert!(validation_result.is_ok(), "Validation should succeed with checksum disabled");
        
        // Step 6: Commit update
        assert!(transaction.commit(&test_binary_path).await.is_ok());
        
        // Complete workflow should succeed
        assert!(true, "Complete validation workflow completed successfully");
    }
}